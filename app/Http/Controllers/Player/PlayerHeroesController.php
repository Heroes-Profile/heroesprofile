...
class PlayerHeroesController extends Controller
{
    ...
    public function getHeroData(Request $request){
        ...
        $result = Replay::join('player', 'player.replayID', '=', 'replay.replayID')
                ...
                ->when($type == "single", function ($query) use ($hero) {
        ...
        if($type == "single"){
            ...
            $ud_mmr_data = MasterMMRDataUD::select("conservative_rating", "win", "loss")->filterByType($hero)->filterByGametype(2)->filterByBlizzID($blizz_id)->filterByRegion($region)->first();
            if($ud_mmr_data){
                $ud_mmr_data->rank_tier = $this->globalDataService->calculateSubTier($this->globalDataService->getRankTiers(2, $hero), $ud_mmr_data->mmr);
            }

            $hl_mmr_data = MasterMMRDataHL::select("conservative_rating", "win", "loss")->filterByType($hero)->filterByGametype(3)->filterByBlizzID($blizz_id)->filterByRegion($region)->first();
            if($hl_mmr_data){
                $hl_mmr_data->rank_tier = $this->globalDataService->calculateSubTier($this->globalDataService->getRankTiers(3, $hero), $hl_mmr_data->mmr);
            }

            $tl_mmr_data = MasterMMRDataTL::select("conservative_rating", "win", "loss")->filterByType($hero)->filterByGametype(4)->filterByBlizzID($blizz_id)->filterByRegion($region)->first();
            if($tl_mmr_data){
                $tl_mmr_data->rank_tier = $this->globalDataService->calculateSubTier($this->globalDataService->getRankTiers(4, $hero), $tl_mmr_data->mmr);
            }

            $sl_mmr_data = MasterMMRDataSL::select("conservative_rating", "win", "loss")->filterByType($hero)->filterByGametype(5)->filterByBlizzID($blizz_id)->filterByRegion($region)->first();
            if($sl_mmr_data){
                $sl_mmr_data->rank_tier = $this->globalDataService->calculateSubTier($this->globalDataService->getRankTiers(5, $hero), $sl_mmr_data->mmr);
            }
            $ar_mmr_data = MasterMMRDataAR::select("conservative_rating", "win", "loss")->filterByType($hero)->filterByGametype(6)->filterByBlizzID($blizz_id)->filterByRegion($region)->first();
            if($ar_mmr_data){
                $ar_mmr_data->rank_tier = $this->globalDataService->calculateSubTier($this->globalDataService->getRankTiers(6, $hero), $ar_mmr_data->mmr);
            }
