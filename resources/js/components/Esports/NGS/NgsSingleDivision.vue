    ...
    <div v-if="data">
      ...
      <img :src="'/images/NGS/Divisions/Division ' + division + '.png'"/>
            ...
            <div class="flex flex-wrap gap-2">
              <round-image v-for="(item, index) in data.teams" :size="'big'" :image="item.image" :showTooltip="true" :hovertextstyleoverride="true">
                <div class="absolute hidden bottom-11 -left-24  bg-gray-dark  text-xs p-1  group-hover:block  text-white z-50 drop-shadow-md w-60 rounded-md px-2 text-center">
                  ...
                  <p>Win Rate: {{ item.win_rate }}</p>
                  <p>Games Played: {{ item.wins + item.losses }}</p>
                </div>
              </round-image>
            </div>
            ...
            <div class="bg-lighten p-10 ">
              ...
              <div class="flex flex-wrap gap-2">
                <hero-image-wrapper v-for="(item, index) in data.heroes" :size="'big'" :hero="item.hero"></hero-image-wrapper>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <map-image-wrapper v-for="(item, index) in data.maps" :size="'big'" :map="item.map"></map-image-wrapper>
              </div>
            </div>
            ...
            <div class="p-10 max-w-[90em] ml-auto mr-auto">
              <h2 class="text-3xl font-bold py-5">Most Recent matches</h2>
                ...
                <!--<game-summary-box v-for="(item, index) in data.matches" :data="item" :caption="`${item.game_map.name} | ${item.game_date}`"></game-summary-box>-->
              
                <custom-button class="flex justify-end " text="View Match History"></custom-button>
