<template>
  <div class="max-w-[1500px] mx-auto mb-4 p-4 rounded bg-lighten text-gray-light">
    ...
    <div v-else-if="notes">
      ...
      <div v-if="notes.bug_fixes && notes.bug_fixes.length">
        ...
        <ul v-if="showBugFixes" class="list-disc list-inside text-sm space-y-1">
          <li v-for="(note, index) in notes.bug_fixes" :key="index">{{ note }}</li>
        </ul>
      </div>
    </div>
